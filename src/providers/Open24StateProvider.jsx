    ...
    const p = (async () => {
      try {
        ...
        if (arr.length === 0 && cafesRef.current.length > 0) {
          setErrMsg("서버가 일시적으로 빈 응답을 줬어. 기존 목록을 유지할게!");
        ...
        if (msg.includes("GEO_NOT_SUPPORTED")) setErrMsg("이 브라우저는 위치 기능을 지원하지 않아.");
        else if (msg.includes("permission") || msg.includes("denied"))
          setErrMsg("위치 권한이 꺼져 있어. 허용 후 다시 시도해줘.");
        else setErrMsg("24시간 카페를 불러오는 중 오류가 발생했어. 잠시 후 다시 시도해줘.");
        return cafesRef.current;
      } finally {
